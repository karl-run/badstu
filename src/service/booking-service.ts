...
export async function getDropins(name: Location): Promise<LocationResult> {
  ...
  if (location == null || (location.dropins_polled_at == null && location.private_polled_at == null)) {
  ...
  return {
    timestamp: R.pipe([location.dropins_polled_at, location.private_polled_at], R.filter(R.isTruthy), min, formatISO),
    result: R.pipe(
      dropins,
      addEmptyDays(diffInDaysToNow ?? 10, createEmptyDropinDay(locations[name].dropinSlots), locations[name].fillDays),
      R.map(
...
function addEmptyDays(daysToAdd: number, emptyDay: AvailabilityMap, daysToFill?: number[]) {
  return (days: ExtractedDay[]): DateTimesTuple[] => {
    ...
    const emptyDays: Record<string, AvailabilityMap> = R.pipe(
      ...
      R.filter(([date]) => (daysToFill == null ? true : daysToFill.includes(getDayCorrect(parseISO(date))))),
      (it) => R.fromEntries(it),
    )
