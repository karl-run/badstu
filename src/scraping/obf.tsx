...
export async function scrapeTimes(name: Location): Promise<void> {
  ...
  console.info(`Scraping ${name}, dropin: ${location.dropin ?? 'true'}, private: ${location.privat ?? 'true'}`)
  ...
  const privateDays: ExtractedDay[] = privateDocuments.map(
    (doc): ExtractedDay => ({
      ...
      times: R.pipe(
        doc.slots,
        R.map((slot): [string, number] => [decimalTimeToStringTime(slot.time), slot.available - slot.booked]),
